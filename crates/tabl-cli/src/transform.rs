...
fn parse_new_column_expr(col_spec: &str) -> Result<Expr, TablCliError> {
...
fn parse_dtype(type_str: &str) -> Result<DataType, TablCliError> {
    match type_str.to_lowercase().as_str() {
        ...
        _ => Err(TablCliError::Error("invalid data type".to_string())),
    }
}

fn create_value_expr(value: &str, dtype: &DataType) -> Result<Expr, TablCliError> {
    match dtype {
        DataType::Int8 => Ok(lit(
            i8::from_str(value).map_err(|_| TablCliError::Error(value.to_string()))?
        )),
        DataType::Int16 => Ok(lit(
            i16::from_str(value).map_err(|_| TablCliError::Error(value.to_string()))?
        )),
        DataType::Int32 => Ok(lit(
            i32::from_str(value).map_err(|_| TablCliError::Error(value.to_string()))?
        )),
        DataType::Int64 => Ok(lit(
            i64::from_str(value).map_err(|_| TablCliError::Error(value.to_string()))?
        )),
        DataType::UInt8 => Ok(lit(
            u8::from_str(value).map_err(|_| TablCliError::Error(value.to_string()))?
        )),
        DataType::UInt16 => Ok(lit(
            u16::from_str(value).map_err(|_| TablCliError::Error(value.to_string()))?
        )),
        DataType::UInt32 => Ok(lit(
            u32::from_str(value).map_err(|_| TablCliError::Error(value.to_string()))?
        )),
        DataType::UInt64 => Ok(lit(
            u64::from_str(value).map_err(|_| TablCliError::Error(value.to_string()))?
        )),
        DataType::Float32 => Ok(lit(
            f32::from_str(value).map_err(|_| TablCliError::Error(value.to_string()))?
        )),
        DataType::Float64 => Ok(lit(
            f64::from_str(value).map_err(|_| TablCliError::Error(value.to_string()))?
        )),
        DataType::Boolean => Ok(lit(
            bool::from_str(value).map_err(|_| TablCliError::Error(value.to_string()))?
        )),
        DataType::String => Ok(lit(value.to_string())),
        DataType::Date => {
            let naive_date =
                chrono::NaiveDate::parse_from_str(value, "%Y-%m-%d").map_err(|_| {
                    TablCliError::Error("set default date string as %Y-%m-%d".to_string())
                })?;
            Ok(lit(naive_date
                .and_hms_opt(0, 0, 0)
                .ok_or_else(|| TablCliError::Error("Failed to create NaiveDateTime".to_string()))?
                .and_utc()
                .timestamp_millis()))
        }
        DataType::Datetime(_, _) => {
            let naive_datetime = chrono::NaiveDateTime::parse_from_str(value, "%Y-%m-%d %H:%M:%S")
                .map_err(|_| TablCliError::Error(value.to_string()))?;
            Ok(lit(naive_datetime.and_utc().timestamp_millis()))
        }
        _ => Err(TablCliError::Error("Unsupported dtype".to_string())),
    }
}
...
fn apply_single_filter(
    ...
    if filter.contains("!=") {
        ...
        Err(TablCliError::Error("Invalid filter format".to_string()))
    }
}
...
fn apply_comparison_filter(
    ...
    if parts.len() != 2 {
        return Err(TablCliError::Error("Invalid filter format".to_string()));
    }

    let (column, value) = (parts[0], parts[1]);
    let column_type = schema
        .get(column)
        .ok_or_else(|| TablCliError::Error(format!("Column '{}' not found", column)))?;

    let filter_expr = match column_type {
        DataType::Binary => {
            if let Some(hex_value) = value.strip_prefix("0x") {
                let binary_value = hex::decode(hex_value)
                    .map_err(|e| TablCliError::Error(format!("Invalid hex value: {}", e)))?;
                if operator == "=" {
        ...
        DataType::UInt64 | DataType::Int64 | DataType::UInt32 | DataType::Int32 => {
            let int_value = if let Some(hex_value) = value.strip_prefix("0x") {
                i64::from_str_radix(hex_value, 16)
                    .map_err(|e| TablCliError::Error(format!("Invalid hex integer: {}", e)))?
            } else {
                value
                    .parse::<i64>()
                    .map_err(|e| TablCliError::Error(format!("Invalid integer: {}", e)))?
            };
            if operator == "=" {
            ...
            let (existing, new): (Vec<String>, Vec<String>) =
                rename
                    ...
                    .try_fold((Vec::new(), Vec::new()), |(mut old, mut new), r| {
                        ...
                        if parts.len() != 2 {
                            return Err(TablCliError::Error("Invalid rename format".to_string()));
                        }
    ...
    match cast {
        ...
        Some(cast) => {
            ...
            for c in cast {
                ...
                if parts.len() != 2 {
                    return Err(TablCliError::Error("InvalidCastFormat".to_string()));
                }
...
pub(crate) fn apply_set(lf: LazyFrame, set: Option<&[String]>) -> Result<LazyFrame, TablCliError> {
    match set {
        None => Ok(lf),
        Some(set) => {
            let mut new_lf = lf;
            ...
            for s in set {
                ...
                println!("parts: {:?}", parts);
                if parts.len() != 2 {
                    return Err(TablCliError::Error("Invalid set format".to_string()));
                }
                let (column, value) = (parts[0], parts[1]);
...
fn raw_str_to_lit(column: &str, value: &str, dtype: &DataType) -> Result<Expr, TablCliError> {
    let lit_value = match dtype {
        DataType::Int8 => lit(i8::from_str(value)
            .map_err(|_| TablCliError::Error(format!("Invalid i8 value: {}", value)))?),
        DataType::Int16 => lit(i16::from_str(value)
            .map_err(|_| TablCliError::Error(format!("Invalid i16 value: {}", value)))?),
        DataType::Int32 => lit(i32::from_str(value)
            .map_err(|_| TablCliError::Error(format!("Invalid i32 value: {}", value)))?),
        DataType::Int64 => lit(i64::from_str(value)
            .map_err(|_| TablCliError::Error(format!("Invalid i64 value: {}", value)))?),
        DataType::UInt8 => lit(u8::from_str(value)
            .map_err(|_| TablCliError::Error(format!("Invalid u8 value: {}", value)))?),
        DataType::UInt16 => lit(u16::from_str(value)
            .map_err(|_| TablCliError::Error(format!("Invalid u16 value: {}", value)))?),
        DataType::UInt32 => lit(u32::from_str(value)
            .map_err(|_| TablCliError::Error(format!("Invalid u32 value: {}", value)))?),
        DataType::UInt64 => lit(u64::from_str(value)
            .map_err(|_| TablCliError::Error(format!("Invalid u64 value: {}", value)))?),
        DataType::Float32 => lit(f32::from_str(value)
            .map_err(|_| TablCliError::Error(format!("Invalid f32 value: {}", value)))?),
        DataType::Float64 => lit(f64::from_str(value)
            .map_err(|_| TablCliError::Error(format!("Invalid f64 value: {}", value)))?),
        DataType::Boolean => lit(bool::from_str(value)
            .map_err(|_| TablCliError::Error(format!("Invalid boolean value: {}", value)))?),
        DataType::String => lit(value.to_string()),
        DataType::Date => {
            let naive_date =
                chrono::NaiveDate::parse_from_str(value, "%Y-%m-%d").map_err(|_| {
                    TablCliError::Error("Invalid date format. Use YYYY-MM-DD".to_string())
                })?;
            lit(naive_date
                .and_hms_opt(0, 0, 0)
                .ok_or_else(|| TablCliError::Error("Failed to create NaiveDateTime".to_string()))?
    ...
    match raw_values {
        ...
        Some(values) => {
            ...
            for value in values.iter() {
                ...
                if parts.len() != 2 {
                    return Err(TablCliError::Error("Invalid format".to_string()));
                }
...
pub(crate) fn apply_head(lf: LazyFrame, n: Option<usize>) -> Result<LazyFrame, TablCliError> {
    match n {
        None => Ok(lf),
...
pub(crate) fn apply_tail(lf: LazyFrame, n: Option<usize>) -> Result<LazyFrame, TablCliError> {
    match n {
        None => Ok(lf),
...
pub(crate) fn apply_offset(lf: LazyFrame, n: Option<usize>) -> Result<LazyFrame, TablCliError> {
    match n {
        None => Ok(lf),
...
pub(crate) fn apply_value_counts(
    lf: LazyFrame,
    n: Option<&str>,
) -> Result<LazyFrame, TablCliError> {
    match n {
