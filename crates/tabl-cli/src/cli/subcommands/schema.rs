...
use std::sync::Arc;
use tabl_core::formats::{format_bytes, format_with_commas};
use tabl_core::parquet::{combine_tabular_summaries, summarize_by_schema, TabularSummary};
use toolstr::Colorize;

pub(crate) async fn schema_command(args: SchemaArgs) -> Result<(), TablCliError> {
    // get schemas
    let paths = tabl_core::filesystem::get_input_paths(&args.paths, args.tree, true)?;
    let summaries = tabl_core::parquet::get_parquet_summaries(&paths).await?;
    let ref_summaries: Vec<&tabl_core::parquet::TabularSummary> = summaries.iter().collect();
    ...
    let paths = if args.absolute {
        ...
        let common_prefix = tabl_core::filesystem::get_common_prefix(&paths)?;
